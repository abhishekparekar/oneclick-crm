      ...
      <div className="h-8 w-14 opacity-60 group-hover:opacity-100 transition-opacity pointer-events-none flex-shrink-0">
        <ResponsiveContainer width="100%" height="100%" minWidth={1} minHeight={1}>
          <AreaChart data={sparkData} margin={{ top: 0, right: 0, left: 0, bottom: 0 }}>
            <defs>
              <linearGradient id={`sk-t-${label.replace(/\s+/g, '')}`} x1="0" y1="0" x2="0" y2="1">
                <stop offset="0%" stopColor={strokeColor} stopOpacity={0.3}/>
                <stop offset="100%" stopColor={strokeColor} stopOpacity={0}/>
              </linearGradient>
            </defs>
            <Area type="monotone" dataKey="v" stroke={strokeColor} strokeWidth={2} fill={`url(#sk-t-${label.replace(/\s+/g, '')})`}/>
          </AreaChart>
        </ResponsiveContainer>
      </div>
    </div>
  ...
  return (
    <div className="space-y-4 pb-12 font-sans text-slate-900 dark:text-slate-100 max-w-[1440px] mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pt-1">
        <div>
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400 font-medium mt-0.5">
            Monitor and assign work across team members
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
        <KPICard label="Total Tasks"     value={totalCount}     trend="16.4%" isUp period="last month" strokeColor="#EAB308" Icon={CheckSquare} iconBg="bg-amber-500/10"  iconColor="#D97706"/>
        <KPICard label="Pending Tasks"   value={pendingCount}   trend="7.3%"  isUp period="last month" strokeColor="#06B6D4" Icon={Clock}        iconBg="bg-cyan-500/10"   iconColor="#0891B2"/>
        <KPICard label="In Progress"     value={inProgressCount} trend="14.2%" isUp period="last month" strokeColor="#8B5CF6" Icon={Sparkles}     iconBg="bg-purple-500/10" iconColor="#7C3AED"/>
        <KPICard label="Completed"       value={completedCount} trend="21.0%" isUp period="last month" strokeColor="#10B981" Icon={CheckCircle}  iconBg="bg-emerald-500/10" iconColor="#059669"/>
        <KPICard label="Overdue Tasks"   value={overdueCount}   trend="2.1%"  isUp={false} period="yesterday" strokeColor="#F43F5E" Icon={AlertTriangle} iconBg="bg-rose-500/10" iconColor="#E11D48"/>
      </div>
        ...
        <div className="bg-white dark:bg-[#111C24] rounded-2xl border border-slate-200/80 dark:border-slate-800 overflow-hidden shadow-2xs">
          <div className="px-4 py-3 border-b border-slate-200/80 dark:border-slate-800 flex items-center justify-between bg-slate-50/50 dark:bg-slate-900/40">
            <h3 className="font-extrabold text-slate-900 dark:text-white text-xs tracking-wider uppercase flex items-center gap-2">
              <Layers size={14} className="text-amber-500" /> Team Tasks Log
            </h3>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800/80">
                {filteredTasks.map((task) => {
                  const status = (task.status || "pending").toLowerCase();
                  const assignee = task.assignedTo?.name || task.assignee?.name || "Unassigned";
                  return (
                    <tr key={task._id} onClick={() => navigate(`/manager/tasks/${task._id}`)} className="border-b border-slate-100 dark:border-slate-800/80 hover:bg-slate-50/80 dark:hover:bg-slate-800/40 transition-colors cursor-pointer group">
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1.5">
                          <MiniAvatar name={assignee} size="w-5 h-5" textSize="text-[9px]" />
...
export default ManagerTeamTasks;
