...
import { getManagerTeamApi, updateManagerProjectApi } from "../../api/managerApi";
import { getEmployeesApi, getDepartmentsApi, updateProjectApi } from "../../api/companyAdminApi";
import {
  FolderKanban,
  Building2,
  UserCheck,
...
const STATUS_OPTIONS = [
  ...
  { value: "completed", label: "Completed", color: "bg-emerald-500/15 text-emerald-700 dark:text-emerald-300 border-emerald-300 dark:border-emerald-800", dot: "bg-emerald-500" },
];
...
export default function ProjectEditModal({ isOpen, onClose, editData }) {
  const { user } = useAuth();
  ...
  useEffect(() => {
    if (editData && isOpen) {
      setForm({
        ...
        startDate: editData.startDate ? new Date(editData.startDate).toISOString().split("T")[0] : getTodayDateString(),
        ...
        nextFollowUpDate: editData.nextFollowUpDate ? new Date(editData.nextFollowUpDate).toISOString().split("T")[0] : "",
        members: Array.isArray(editData.members) ? editData.members.map((m) => m._id || m) : [],
  ...
  const updateMut = useMutation({
    mutationFn: (data) =>
      isAdmin ? updateProjectApi(editData._id, data) : updateManagerProjectApi(editData._id, data),
    onSuccess: () => {
      ...
      >
        ...
        <div className="flex items-center justify-between px-4 sm:px-6 py-3.5 border-b border-slate-200/80 dark:border-slate-800/80 bg-slate-50/80 dark:bg-[#111C2D] flex-shrink-0">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-cyan-500 to-cyan-600 dark:from-cyan-500 dark:to-cyan-700 flex items-center justify-center text-white shadow-sm flex-shrink-0">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-md bg-cyan-500/10 text-cyan-600 dark:text-cyan-400 border border-cyan-500/20">
                  {form.status}
                </span>
              </div>
              <p className="text-[11px] text-slate-500 dark:text-slate-400 leading-none mt-0.5">
                Update parameters, reassign team members, and modify timeline milestones.
              </p>
            </div>
          </div>
            ...
            <div className="lg:col-span-7 space-y-3.5">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 bg-white dark:bg-[#152238] border border-slate-300 dark:border-slate-700 rounded-xl text-xs font-semibold text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-cyan-500/40 focus:border-cyan-500 transition-all shadow-xs"
                />
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full px-3 py-2 bg-white dark:bg-[#152238] border border-slate-300 dark:border-slate-700 rounded-xl text-xs font-semibold text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-cyan-500/40 focus:border-cyan-500 transition-all cursor-pointer shadow-xs"
                  >
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full px-3 py-2 bg-white dark:bg-[#152238] border border-slate-300 dark:border-slate-700 rounded-xl text-xs font-medium text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-cyan-500/40 focus:border-cyan-500 transition-all resize-none shadow-xs"
                />
              </div>
              ...
              <div className="bg-slate-50 dark:bg-[#111C2D] border border-slate-200/80 dark:border-slate-800/80 p-3 rounded-xl space-y-2.5">
                <div className="flex items-center justify-between text-[11px] font-bold uppercase tracking-wider text-slate-700 dark:text-slate-300">
                  <span className="flex items-center gap-1">
                    <CalendarDays size={12} className="text-cyan-500" /> Timeline & Schedule
                  </span>
                ...
                <div className="grid grid-cols-2 gap-2.5">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-2.5 py-1.5 bg-white dark:bg-[#152238] border border-slate-300 dark:border-slate-700 rounded-lg text-xs font-semibold text-slate-800 dark:text-white focus:outline-none focus:ring-1 focus:ring-cyan-500 shadow-2xs"
                    />
                  </div>
              ...
              <div className="space-y-1.5">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2 text-[10px]">
                    <button
                      ...
                      className="text-cyan-600 hover:text-cyan-700 dark:text-cyan-400 font-bold cursor-pointer"
                    >
                      Select All
                    </button>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-7 pr-3 py-1.5 bg-white dark:bg-[#152238] border border-slate-300 dark:border-slate-700 rounded-lg text-xs font-semibold text-slate-800 dark:text-white placeholder-slate-400 focus:outline-none focus:ring-1 focus:ring-cyan-500 shadow-2xs"
                  />
                </div>
                      ...
                      return (
                        <div
                          ...
                          className={`flex items-center justify-between p-1.5 rounded-lg text-xs cursor-pointer transition-all ${
                            isSelected
                              ? "bg-cyan-500/15 border border-cyan-500/30 text-cyan-900 dark:text-cyan-200 font-bold"
          ...
          <div className="pt-3 border-t border-slate-200/80 dark:border-slate-800/80 flex items-center justify-between bg-white dark:bg-[#0E1726]">
            <div className="text-[11px] text-slate-400 hidden sm:block">
              <span className="text-emerald-600 dark:text-emerald-400 font-semibold flex items-center gap-1">
                <CheckCircle2 size={12} /> Modifications ready to save
              </span>
            </div>
            ...
            <div className="flex items-center gap-2.5 ml-auto">
              ...
              <button
                ...
                className="px-5 py-2 bg-gradient-to-r from-cyan-600 to-cyan-700 hover:from-cyan-700 hover:to-cyan-800 text-white rounded-xl text-xs font-extrabold transition-all shadow-md hover:shadow-lg disabled:opacity-50 cursor-pointer flex items-center gap-1.5"
              >
                {updateMut.isPending ? (
                  <>
