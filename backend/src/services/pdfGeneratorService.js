...
const generatePayslipHTML = (payroll, company, settings = {}) => {
  ...
  const calculatedGross = earningRows.reduce((sum, [, v]) => sum + (Number(v) || 0), 0);
  const grossEarnings = (earn.grossEarnings !== undefined && earn.grossEarnings > 0)
    ? earn.grossEarnings
    : (calculatedGross || p.grossSalary || 0);

  const calculatedDeductions = deductionRows.reduce((sum, [, v]) => sum + (Number(v) || 0), 0);
  const totalDeductions = (ded.totalDeductions !== undefined && ded.totalDeductions > 0)
    ? ded.totalDeductions
    : calculatedDeductions;

  const netSalary = p.netSalary !== undefined && p.netSalary > 0
    ? p.netSalary
    : Math.max(0, grossEarnings - totalDeductions);

  const words = p.amountInWords || numberToWords(netSalary);
...
<div class="payslip-container">
  ...
  <div class="net-box">
    <div>
      ...
      <div class="net-words">Amount in Words: <b>${words}</b></div>
    </div>
