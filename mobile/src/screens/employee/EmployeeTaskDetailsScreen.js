...
const EmployeeTaskDetailsScreen = ({ route, navigation }) => {
  ...
  const handleDirectFileUpload = async () => {
    try {
      ...
      const uploadRes = await uploadTaskAttachmentApi(taskId, formData).catch(async () => {
        return await uploadMediaFileApi(formData);
      });
  ...
  const handleToggleChecklist = async (checkItem, idx) => {
    ...
    setUpdatingChecklistId(checkItem._id || idx);
    ...
    try {
      ...
      if (commentFile) {
        ...
        formData.append("file", {
          ...
          type: commentFile.type || "application/octet-stream"
        });

        const uploadRes = await uploadMediaFileApi(formData).catch(async () => {
        ...
        if (resData && (resData.fileUrl || resData.url || resData.attachment?.fileUrl)) {
          attachmentsList.push({
            ...
            fileName: resData.fileName || resData.filename || resData.attachment?.fileName || commentFile.name || "Attachment",
            fileType: resData.fileType || resData.attachment?.fileType || commentFile.type || "application/octet-stream"
          });
        }
      ...
      await addTaskCommentApi(taskId, { comment: commentToSend, attachments: attachmentsList });
      setNewComment("");
      setCommentFile(null);
      await fetchTaskDetails(false);
    } catch (err) {
              ...
              <View style={styles.nativeChecklistSection}>
                ...
                {checklistItems.map((item, idx) => (
                  <TouchableOpacity
                    ...
                    disabled={updatingChecklistId === (item._id || idx)}
                    activeOpacity={0.8}
                  >
                    <Ionicons
