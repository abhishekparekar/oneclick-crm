...
const EmployeeTaskDetailsScreen = ({ route, navigation }) => {
  ...
  const handleToggleChecklist = async (checkItem) => {
    if (!task) return;
    setUpdatingChecklistId(checkItem._id);
    try {
      const newStatus = !checkItem.isCompleted;
      const res = await updateTaskChecklistApi(taskId, checkItem._id, { isCompleted: newStatus });
      if (res.data && res.data.success) {
        setTask((prevTask) => {
          if (!prevTask) return prevTask;
          const updatedChecklist = (prevTask.checklist || []).map((item) =>
            item._id === checkItem._id ? { ...item, isCompleted: newStatus } : item
          );
  ...
  const playAudioPreview = async () => {
    if (!commentFile || !commentFile.uri) return;
    try {
      if (sound) {
        if (isPlayingAudio) {
  ...
  const handleAddComment = async () => {
    ...
    try {
      ...
      if (commentFile) {
        ...
        const uploadRes = await uploadTaskAttachmentApi(taskId, formData).catch(() => null);
        if (uploadRes && uploadRes.data && uploadRes.data.success && uploadRes.data.attachment) {
          attachmentsList.push(uploadRes.data.attachment);
        }
        setUploadingCommentMedia(false);
      }

      await addTaskCommentApi(taskId, { comment: newComment, attachments: attachmentsList });
      setNewComment("");
      setCommentFile(null);
  ...
  if (!deptName || deptName === "[object Object]") deptName = "My Department";

  const checklistItems = task.checklist || [];
  const completedChecklistCount = checklistItems.filter(c => c.isCompleted).length;
    ...
    <View style={styles.container}>
      ...
      >
        ...
        <View style={styles.headerTitleWrap}>
          <Text style={styles.headerTitleText}>My Task Details</Text>
          {task.taskId && (
            ...
            <View style={styles.nativeInfoGrid}>
              ...
              <View style={styles.nativeGridCell}>
                ...
                <View style={{ flex: 1 }}>
                  ...
                  <Text style={styles.nativeGridVal} numberOfLines={1}>{task.assignedBy?.name || "Manager / Admin"}</Text>
                </View>
              </View>
              ...
              <View style={styles.nativeChecklistSection}>
                <View style={styles.nativeChecklistHeader}>
                  <View style={{ flexDirection: "row", alignItems: "center" }}>
                    ...
                    <Text style={styles.nativeSectionLabel}>MY TASK CHECKLIST</Text>
                  </View>
                  <Text style={styles.nativeProgressText}>
                    {completedChecklistCount}/{checklistItems.length} Done ({Math.round((completedChecklistCount / checklistItems.length) * 100)}%)
                  </Text>
                </View>
                ...
                {checklistItems.map((item, idx) => (
                  <TouchableOpacity
                    ...
                    disabled={updatingChecklistId === item._id}
                    activeOpacity={0.8}
                  >
                    <Ionicons
            ...
            <View style={styles.nativeQuickRow}>
              ...
              {!isCompleted && !isCancelled && (
                <TouchableOpacity
                  ...
                  onPress={() => openActionModal(isOverdue ? "late-complete" : "complete")}
                  activeOpacity={0.8}
                >
        ...
        <View style={[styles.stickyCommentBar, { paddingBottom: Math.max(insets.bottom, 12) + (Platform.OS === 'android' ? keyboardHeight : 0) }]}>
          {commentFile && (
            <View style={styles.attachedPreviewRow}>
              {commentFile.type?.startsWith('audio') ? (
              ...
              <TouchableOpacity onPress={removeCommentFile} style={{ padding: 4 }}>
          ...
          <View style={styles.commentInputRow}>
            ...
            <TouchableOpacity style={styles.mediaActionBtn} onPress={startRecording} disabled={isRecording || !!commentFile}>
            ...
            <TouchableOpacity 
              style={[
                ...
                (!newComment.trim() && !commentFile) && { backgroundColor: '#E2E8F0' }
              ]} 
              onPress={handleAddComment} 
              disabled={submittingComment || uploadingCommentMedia || (!newComment.trim() && !commentFile) || isRecording}
              activeOpacity={0.85}
            >
              {submittingComment || uploadingCommentMedia ? (
                ...
                <Ionicons
                  name="send"
                  size={18}
                  color={(!newComment.trim() && !commentFile) ? "#94A3B8" : "#FFFFFF"}
                  style={{ marginLeft: 2 }}
                />
              )}
            </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  // ── Seamless Action Bar (No Card Box) ──
  nativeActionBar: {
    paddingBottom: 10,
    borderBottomWidth: 1,
    borderBottomColor: "#F1F5F9",
