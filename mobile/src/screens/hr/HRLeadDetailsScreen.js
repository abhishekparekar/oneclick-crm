...
export default function HRLeadDetailsScreen({ route, navigation }) {
  ...
  // Status Change Modal & Follow-up State
  ...
  const fetchDetails = async () => {
    try {
      ...
      const [leadData, statusList, assignableUsers, tplList] = await Promise.all([
        leadId
          ? leadsService.getLeadById(leadId).catch((err) => {
              console.warn("[HRLeadDetails] getLeadById error:", err?.message || err);
              return null;
            })
      ...
      if (Array.isArray(tplList) && tplList.length > 0) {
        ...
        if (!selectedTemplate) {
          ...
          initVariablesForTemplate(tplList[0], effectiveLead);
        }
      }
              ...
              <View style={styles.compactDetailsCard}>
                  ...
                  <View style={[styles.compactCell, { flex: 1.4 }]}>
                    ...
                    <View style={styles.compactValueRow}>
                      ...
                      <Text style={[styles.compactCellValue, { color: "#4F46E5" }]} numberOfLines={1}>
                        {assignedRepName
                          ? `${assignedRepName}${assignedRepDept ? ` (${assignedRepDept})` : ""}`
                          : "Unassigned"}
                      </Text>
                    </View>
                  </View>
                ...
                <View style={[styles.compactGridRow, { borderTopWidth: 1, borderTopColor: "#F1F5F9" }]}>
                  ...
                  <View style={[styles.compactCell, { flex: 0.9 }]}>
                    ...
                    <Text style={[styles.compactCellValue, { color: lead.estimatedValue ? THEME.emerald : THEME.textMuted, fontFamily: FONTS.displayBold }]} numberOfLines={1}>
                    ...
                    <View style={styles.compactValueRow}>
                      ...
                      <Text
                        style={[
                          ...
                          lead.nextFollowUpDate ? { color: "#7C3AED", fontFamily: FONTS.displayBold } : { color: THEME.textMuted },
                        ]}
                        numberOfLines={1}
                      >
                        {lead.nextFollowUpDate
